...
def run_truncation_case(
    *,
    case_id: str,
    intent: str,
    truncation_value: str | None,
    ...
    try:
        ...
        if truncation_value is not None:
            kwargs["truncation"] = truncation_value
        ...
        return {
            ...
            "actual_model": "",
            "sent_truncation": truncation_value if truncation_value is not None else "<absent>",
            "response_truncation": "",
            "response_status": "",
            "output_item_types": [],
    ...
    text = output_text(response, payload)
    expected = "truncation-probe-ok"
    status = "supported" if expected in text else "accepted_unexpected_answer"
    return {
        "case": case_id,
        "intent": intent,
        "status": status,
        ...
        "sent_truncation": truncation_value if truncation_value is not None else "<absent>",
        "response_truncation": payload.get("truncation"),
...
def write_result_files(title: str, result_json_path: Path, result_md_path: Path, result: dict[str, Any]) -> None:
    ...
    summary_rows = [
        [
            ...
            result["status"],
            result["actual_model"],
            result["sent_truncation"],
            result["response_truncation"],
