...
RESPONSE_JSON_PATH = OUTPUT_DIR / "truncation_disabled_response.json"
RESULT_JSON_PATH = OUTPUT_DIR / "truncation_disabled_results.json"
RESULT_MD_PATH = OUTPUT_DIR / "truncation_disabled_results.md"
...
def main() -> int:
    ...
    print("intent: verify whether Codex backend accepts the official truncation=disabled value")

    result = run_truncation_case(
        case_id="truncation_disabled",
        intent="explicit official truncation=disabled",
        truncation_value="disabled",
        response_json_path=RESPONSE_JSON_PATH,
    )
    write_result_files("Truncation Disabled Results", RESULT_JSON_PATH, RESULT_MD_PATH, result)
