...
RESPONSE_JSON_PATH = OUTPUT_DIR / "max_tool_calls_zero_response.json"
RESULT_JSON_PATH = OUTPUT_DIR / "max_tool_calls_zero_results.json"
RESULT_MD_PATH = OUTPUT_DIR / "max_tool_calls_zero_results.md"
...
def main() -> int:
    ...
    print("value: 0")
    print("intent: verify whether backend accepts max_tool_calls=0 with a required web_search tool")

    result = run_max_tool_calls_case(
        case_id="max_tool_calls_zero",
        intent="max_tool_calls=0 with required web_search",
        max_tool_calls=0,
        prompt=PROMPT,
        response_json_path=RESPONSE_JSON_PATH,
        tool_choice="required",
    )
    write_result_files("Max Tool Calls Zero Results", RESULT_JSON_PATH, RESULT_MD_PATH, result)
