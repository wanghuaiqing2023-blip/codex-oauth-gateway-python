...
def main() -> int:
    ...
    if EXPECTED_TEXT in output_text:
        status = "supported_with_final_output"
        observation = "background=true request returned final output immediately"
    else:
        status = "accepted_without_final_output"
        observation = "background=true request accepted but did not return final output"
