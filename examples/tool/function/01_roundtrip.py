...
RESPONSE1_JSON_PATH = OUTPUT_DIR / "function_tool_roundtrip_response1.json"
RESPONSE2_JSON_PATH = OUTPUT_DIR / "function_tool_roundtrip_response2.json"

TOOL_NAME = "get_probe_value"
EXPECTED_KEY = "gateway_status"
TOOL_OUTPUT = {"probe_status": "gateway-function-roundtrip-ok"}
PROMPT1 = (
    "Call the get_probe_value function with key gateway_status. "
    "Do not answer directly."
)
PROMPT2 = (
    "Use the supplied function output to answer. Reply exactly with the "
    "probe_status value and nothing else."
)

FUNCTION_TOOL = {
    "type": "function",
    "name": TOOL_NAME,
    "description": "Return a deterministic value for the gateway function probe.",
    "strict": True,
    "parameters": {
        "type": "object",
        ...
        "properties": {
            "key": {
                "type": "string",
                "description": "Lookup key for the simulated client-side function.",
            }
        },
        "required": ["key"],
    },
...
def find_function_call(payload: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
...
def make_function_call_context_item(function_call: dict[str, Any]) -> dict[str, Any]:
    ...
    if function_call.get("id"):
        context_item["id"] = function_call["id"]
    if function_call.get("status"):
        context_item["status"] = function_call["status"]
    return context_item
...
def main() -> int:
    ...
    print("intent: verify function_call -> client output -> final model answer")
    print("state_mode: stateless input array; previous_response_id is intentionally not used")
    print(f"expected_key: {EXPECTED_KEY!r}")
    print(f"simulated_tool_output: {json.dumps(TOOL_OUTPUT, ensure_ascii=False)}")
    ...
    match = find_function_call(payload1)
    if not match:
        print("\nRoundtrip result:")
        print_table(
            ["phase", "status", "actual_model", "output_item_types", "elapsed_ms", "observation"],
    ...
    expected_text = TOOL_OUTPUT["probe_status"]
    ...
    print_table(
        ...
        [
            ...
            [
                "phase2_tool_output",
                "supported_roundtrip" if expected_text in final_text else "accepted_without_expected_text",
                ...
                elapsed2_ms,
                call_id,
                "final text contains function output" if expected_text in final_text else "final text did not contain function output",
            ],
        ],
    )

    print("\nDetails:")
    print(f"function_call_path: {function_call_path}")
    print(f"function_arguments: {json.dumps(arguments, ensure_ascii=False)}")
    print(f"argument_key_matches: {arguments.get('key') == EXPECTED_KEY}")
    print(f"response1_json: {RESPONSE1_JSON_PATH}")
