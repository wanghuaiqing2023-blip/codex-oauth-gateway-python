...
CASE = "context_management_compaction_threshold"
EXPECTED_TEXT = "context-management-probe-ok"
...
def main() -> int:
    ...
    print(f"context_management: {json.dumps(CONTEXT_MANAGEMENT, separators=(',', ':'))}")
    ...
    if EXPECTED_TEXT in output_text:
        status = "supported"
        observation = "request accepted with compaction threshold context management"
    else:
