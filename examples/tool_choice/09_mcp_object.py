...
def contains_mcp_evidence(value: Any) -> bool:
    if isinstance(value, dict):
        if value.get("type") in {"mcp_list_tools", "mcp_call", "mcp_approval_request"}:
            return True
        return any(contains_mcp_evidence(child) for child in value.values())
    if isinstance(value, list):
        return any(contains_mcp_evidence(item) for item in value)
    return False
...
def main() -> int:
    ...
    try:
        ...
        found_mcp = contains_mcp_evidence(payload)
        status = "supported_unexpectedly" if found_mcp else "accepted_without_mcp_evidence"
        observation = "found MCP output item" if found_mcp else "request accepted; no MCP output item observed"
    ...
    print("case: tool_choice_mcp_object")
    ...
    print("tool: official MCP with unreachable remote server")
    print("intent: verify whether tool_choice can force an official MCP tool")
