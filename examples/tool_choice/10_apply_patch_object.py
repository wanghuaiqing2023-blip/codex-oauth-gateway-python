...
def contains_apply_patch_call(value: Any) -> bool:
    if isinstance(value, dict):
        if value.get("type") == "apply_patch_call":
            return True
        if value.get("type") == "custom_tool_call" and value.get("name") == "apply_patch":
            return True
        if value.get("name") == "apply_patch":
            return True
        return any(contains_apply_patch_call(child) for child in value.values())
    if isinstance(value, list):
        return any(contains_apply_patch_call(item) for item in value)
    return False
...
def main() -> int:
    ...
    try:
        response = client.responses.create(
            ...
            input=(
                ...
                "examples/tool_choice/tool_choice_apply_patch_probe_target.txt containing exactly one line: "
                "tool-choice-apply-patch-ok. Do not modify real files yourself; only emit the tool call."
            ),
        ...
        found_patch = contains_apply_patch_call(payload)
        status = "supported" if found_patch else "accepted_without_observed_tool_call"
        observation = "found apply_patch call" if found_patch else "request accepted; no apply_patch call observed"
    ...
    print("case: tool_choice_apply_patch_object")
    ...
    print("tool: official apply_patch")
    print("intent: verify whether tool_choice can force the official apply_patch tool")
    print("scope: this probe does not apply any patch")
