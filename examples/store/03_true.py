...
PROMPT = "Reply exactly: store-probe-ok"
EXPECTED_OUTPUT = "store-probe-ok"
...
def main() -> int:
    ...
    try:
        ...
        observation = (
            "request accepted; gateway should still send store=false upstream"
            if EXPECTED_OUTPUT in output_text
            else "response text did not match expected output"
        )
