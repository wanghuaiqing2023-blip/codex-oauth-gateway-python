...
CASE = "non_stream_include_obfuscation_false"
...
def main() -> int:
    ...
    if EXPECTED_TEXT in output_text:
        ...
        observation = "non-stream request accepted; no stream events are expected"
    else:
        status = "accepted_no_stream_evidence"
        observation = "non-stream request accepted but expected output text was not observed"
    ...
    print(f"collected_text: {output_text!r}")
    print(f"status: {status}")
